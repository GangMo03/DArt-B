...
리포트를 볼 사람 입장에서 `register_device = 1` 같은 코드 값은 의미를 알기 어렵기 때문에, 가독성을 위해 코드 값을 레이블로 바꿔주는 작업이 필요하다. 이렇게 **특정 조건을 기반으로 값을 결정할 때는 CASE 식**을 쓴다.
...
사용자가 어떤 웹 페이지를 거쳐 넘어왔는지 판별할 때는 **레퍼러**(지금 이 페이지에 오기 직전에 머물렀던 이전 페이지의 주소)를 집계한다. 그런데 URL 주소를 통째로 보면 가독성이 좋지 않기 때문에, **필요한 특정 값만 추출**해서 쓰는 편이 낫다.
