...
WHERE은 보통 <, >와 같이 일반 조건식을 사용하고 HAVING은 그룹화 이후 AVG()나 SUM 같이 집계 결과를 
필터링 하는데 사용한다.
그렇기 때문에 WHERE로 원본 데이터에서 필터링 해서 GROUP BY로 그룹별로 묶고 HAVING을 사용해 집계함수를
활용해서 결과를 필터링하고 마지막으로 ORDER BY를 통해 최종 결과를 정렬한다.
