...
C:\Users\tkvkm\OneDrive\바탕 화면\Dart_B\Homeworks\images\학습인증.png
...
datetime에서 특정 부분만 추출하여서 나머지 부분과 구분하고 싶은 경우 extract
date와 hour등 특정 정보만 남기고 싶은 경우 시간 자르기인 datetime_trunc 
문자열을 날짜 형식의 datetime 타입으로 바꾸고 싶은 경우 parse_datetime
반대로 datetime 타입을 문자열 데이터로 바꾸고 싶은경우 fromat_datetime
...
특정 조건에 따라 다른 값을 표시하고 싶을떄 사용 
특정 카테고리를 하나로 합치는 전처리가 필요할 수 있기 때문에 
case when은 여러조건이 존재할 경우 사용하고 순서가 중요하다
if는 단일 조건일 경우 사용한다.
...
여기에 답을 작성해주세요!
틀린 쿼리 3번
Date 타입의 데이터는 2000-00-00 형식이다. 이런 컬럼을 문자열 형식과 직접 비교하면 타입 불일치가 발생한다. 그래서 날짜 범위를 명시하거나 날짜함수를 사용해서 연도만 추출해야 한다.
...
Pikachu, Bulbasaur이 Fire도 아니고 Water도 아니므로 Normal이 적용되어 출력됩니다.
