...
1.type2가없는포켓몬의type1과type1의포켓몬수를알려주는쿼리를작성해주세요 단,type1의포켓몬수가큰순으로정렬해주세요
SELECT type1, COUNT(*) AS pokemon_count
FROM pokemon
WHERE type2 IS NULL OR type2 = ''
GROUP BY type1
ORDER BY pokemon_count DESC;

2.전설여부에따른포켓몬수를알수있는쿼리를작성해주세요
SELECT is_legendary, COUNT(*) AS pokemon_count
FROM pokemon
GROUP BY is_legendary;
3.trainer테이블에서“Iris”트레이너의정보를알수있는쿼리를작성해주세요
SELECT *
FROM trainer
WHERE name = 'Iris';
4.전체포켓몬수는얼마나되나요?
...
FROM pokemon;
5.세대(generation)별로포켓몬수가얼마나되는지알수있는쿼리를작성해주세요
SELECT generation, COUNT(*) AS pokemon_count
FROM pokemon
GROUP BY generation
ORDER BY generation;
6.type2가존재하는포켓몬의수는얼마나되나요?
SELECT COUNT(*) AS dual_type_pokemon
FROM pokemon
WHERE type2 IS NOT NULL AND type2 <> '';
7.단일(하나의타입만있는)타입포켓몬중많은type1은무엇일까요?
SELECT type1, COUNT(*) AS single_type_count
FROM pokemon
WHERE type2 IS NULL OR type2 = ''
GROUP BY type1
ORDER BY single_type_count DESC
LIMIT 1;
...
새롭게 나온 GROUP BY ALL 함수를 활용하여
기존보다 훨씬 효율적으로 정리할 수 있다.
...
조건(필터링), 추출, 변환, 요약의 과정으로 정리하는데
무엇을 구해야 하는가를 정하고 SELECT를 통해서
어디서 가져오는지를 확인한 다음 GROUP BY, ORDER BY를
통해서 어떻게 묶고 정렬해야되는지를 정하고 조건이 있는지를 판단한다.
...
C:\Users\tkvkm\OneDrive\바탕 화면\Dart_B\DArt_B-Data_Literacy-1\images\학습_인증(3).png
