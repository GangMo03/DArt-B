...
'기계에 맡길 수 있는일', '사람이 해야 하는 일' 을 구별하고 그에 맡게 기계를 효과적으로
활용해 결과를 창출하는 것이 핵심 능력이다.
...
생각하고-작업하고-생각한다 생각하는 과정은 사람만이 할 수 있는 것이기에
데이터를 분석하기 위해 반드시 필요한 과정이다.
...
이 파트에서 많은 부분을 느낄 수 있었는데 보통 사람들은 데이터가 주어지면 이에 사로잡혀 
앞서 언급한 생각하는 과정을 잊은채 주어진 데이터를 바쁘게 분석하기 바쁘다.
그리고 분석이 끝나면 나온 결과들을 뭉쳐 이리저리 짜맞추다 그냥 결과 제시에 그치거나 이상한 결론에 도달하기도 한다.
이렇기 때문에 우리는 데이터를 분석하기 전에 먼저 목적을 설정하고 그에 따라 데이터를 분석한후 이를 결론으로 연결하는 
과정이 매우 중요한 것 같다.
...
분석 전에 문제 및 목적을 정의하고 가설을 구축한 뒤 데이터를 분석하는 것이 아닌 데이터를 먼저 분석한 후 이에 따른 부실한 결론을 제시하고 있다. 좀 더 올바른 분석이 되려면 분석 전 먼저 문제가 무엇인지 파악한 뒤 가설을 설정하고
응답자들을 어떻게 분류하고 결과를 도출할 것인지 '제품에 만족한다'라고 답했으면 어떤 부분에서 만족한 것인지를 데이터 분석을 통해 파악한 뒤 이를 활용해 문제의 해결방안을 도출하여야 한다.
